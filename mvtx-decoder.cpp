...
struct decoder_t
{
	...
	{
		return (((! _ptr) ? ptr : _ptr) - filebuffer);
	}
};
...
static inline void threshold_next_charge(float* sumd, float* sumd2, int ch, int* lasthist,
		int* hist, int ninj)
{
	int ch1 = ch - 1;
	int ch2 = ch;
...
static inline EXIT_CODE decoder_read_event_into_lanes(decoder_t* decoder)
{
	...
	{ // loop over pages in files
		...
		ASSERT(((! rdh.packetCounter) || (rdh.packetCounter == prev_pck_cnt + 1)), decoder,
		...
		for ( size_t iflx = 0; iflx < nFlxWords; ++iflx )
		{
			...
			for( size_t igbt = 0; igbt < n_gbt_word; ++igbt )
			{
				...
				{ // TRIGGER DATA HEADER (TDH)
					...
					//std::cout << "Enter: if ( (tdh.trigger_type >> Trg::PHYSICS) & 0x1 )" << std::endl;

			
					GlobalL1TrigBCO = tdh.orbit;
					...
					if ( (tdh.trigger_type >> Trg::PHYSICS) & 0x1 )
					{
						...
						//	std::cout << "strobe_id = " << strobe_id << "  YES: Has L1 Trigger " << std::endl;
					...
					//					L1Trig = false;
						...
						//	  std::cout << "strobe_id = " << strobe_id << "  Has NO L1 Trigger " << std::endl;
					}			  


					//L1_StrobeID = strobe_id;
				...
				{ // lane payload
					...
					haspayload = true;
					ASSERT(header_found, decoder,
							"Trigger header not found before chip data, in byte %ld",
							decoder->ptr_pos(flx_ptr));
...
void save_file(std::string fname, uint32_t n_row, uint32_t nChipsPerLane, float*& data)
{
	...
	fileMap.write(reinterpret_cast<const char*>(data),
			n_row * nChipsPerLane * 1024 * sizeof(float) );
	fileMap.close();
}
		...
		const int& n_vcasn_ithr = 1)
{
	...
	float *rmss  = reinterpret_cast<float*>(_mm_malloc( nChipsPerLane * n_row * 1024 * sizeof(float),
				4096) );
	float* sumd  = reinterpret_cast<float*>(_mm_malloc( nChipsPerLane *   1 * 1024 * sizeof(float),
				4096) );
	float* sumd2 = reinterpret_cast<float*>(_mm_malloc( nChipsPerLane *   1 * 1024 * sizeof(float),
				4096) );
	int* rowhist     = reinterpret_cast<int*>(_mm_malloc( (nChipsPerLane * 1024 + 1) * sizeof(int),
				4096) );
	int* lastrowhist = reinterpret_cast<int*>(_mm_malloc( (nChipsPerLane * 1024 + 1) * sizeof(int),
				4096) );
		...
		} else if ( ret == HB_DATA_DONE )
		{
			...
			if (iChg == decoder->thscan_nChg)
			{
				...
				threshold_next_row(thrs + iRow * nChipsPerLane * 1024,
						rmss + iRow * nChipsPerLane * 1024,
						sumd, sumd2,
						decoder->thscan_nChg, decoder->thscan_nInj);
				float m, merr, s, serr;
				meanrms(&m, &merr, thrs + decoder->trigger.thscan_row * nChipsPerLane * 1024,
						nChipsPerLane * 1024);
				meanrms(&s, &serr, rmss + decoder->trigger.thscan_row * nChipsPerLane * 1024,
						nChipsPerLane * 1024);
...
{
	std::cout << "New Codes Brio" << std::endl;
...
//	L1Tree = new TTree("L1Tree", "L1Tree");


  //  L1Tree->Branch("L1_StrobeID",&L1_StrobeID);
//    L1Tree->Branch("L1Trig",&L1Trig);
	
	int event, Nhits;
	uint64_t StrobeID;
	uint64_t BCO;
	uint16_t BC;
	uint64_t L1TrigStrobeID;
	uint64_t L1TrigBCO;
	bool L1Trig;
	...
	tree->Branch("HBID", &HBID);

	tree->Branch("StrobeID", &StrobeID);	
	tree->Branch("BCO", &BCO);
	tree->Branch("L1TrigBCO", &L1TrigBCO);
	tree->Branch("L1Trig", &L1Trig);
	
	tree->Branch("BC", &BC);
	tree->Branch("StrobeNum", &StrobeNum);

	tree->Branch("Nhits", &Nhits);

	tree->Branch("HeaderFEEID", &HeaderFEEID);
	tree->Branch("HeaderStaveID", &HeaderStaveID);
	tree->Branch("HeaderLayerID", &HeaderLayerID);
	...
	while (true)
	{
		...
		} else if ( ret == HB_DATA_DONE )
		{
			...
			//      size_t n_hits = std::accumulate(decoder->m_strobeHits.begin(), decoder->m_strobeHits.end(), 0,
			//          [](auto prev_nHits, auto &entry) { return prev_nHits + entry.second; });
			//      assert( n_hits == nhits );
			transformhits(decoder->m_hits.data(), nhits);
			fillhitmap(hitmap, decoder->m_hits.data(), nhits);


//			std::cout << "Enter: if nHB_with_data" << std::endl;
			...
			{
				StrobeID = strobe.first; //  strobe_bco = strobe_id[51:12], strobe_bc = strobe_id[11:0]        
				BC = StrobeID & 0xFFF;
				BCO = (StrobeID >> 12) & 0xFFFFFFFFFFFF;

				HeaderFEEID = decoder->feeid;
				HeaderStaveID = decoder->feeid & 0x003f;
				HeaderLayerID = (decoder->feeid & 0x7000) >> 12;

			//	L1TrigStrobeID = decoder->trigger.strobe_id;
				L1TrigBCO = GlobalL1TrigBCO;
				L1Trig = GlobalL1Trig;

				Iter hiter_end = hiter_beg + strobe.second;

				//std::cout << "physTrg_bco.size() = " << physTrg_bco.size() << std::endl; 

				for ( Iter hiter = hiter_beg; hiter < hiter_end; ++hiter )
				{
					uint32_t hit = *hiter; 
					uint32_t column = hit & 0x3FF;
					// lane is the next 2 bits [11th and 10th], counting from the right
					uint32_t lane = (hit >> 10) & 0x3;
					// [15th to 12th] are not used
					// [24th to 16th] for row ID (9 bits), counting from the right
					uint32_t row = (hit >> 16) & 0x1FF;
					...
					//	 StrobeNumVec.push_back(StrobeNum);           
			...
			event = static_cast<int>(nHB);
			HBID = static_cast<int>(nHB);
...
//	L1Tree->Write("", TObject::kWriteDelete);
	outfile->Close();
